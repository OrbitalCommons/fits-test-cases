...
| `meter-sim/` | 11 | [meter-sim](https://github.com/CosmicFrontierLabs/meter-sim) | Synthetic test patterns: gradients, checkerboards, Gaussian blobs, mixed types, and extreme aspect ratios. Generated by the meter-sim telescope simulator test suite. |
| `mwa/` | 8 | [MWATelescope](https://github.com/MWATelescope) | MWA radio telescope test data from mwalib, Marlu, Birli, and Hyperdrive. Metafits files, MWAX correlator gpubox data, sky model catalogs, and UVFITS. All MPL-2.0. |
...
- Synthetic test patterns (gradients, checkerboards, Gaussian blobs)
- Extreme aspect ratio images (panoramic, columnar, single-row/column)
- Mixed data types within a single file
- CONTINUE long-string convention
- Array-in-cell binary table columns (repeat count > 1)
- AIPS-convention tables (AN, FQ)
